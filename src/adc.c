...
void adcConfig(void){
	...
	LPC_ADC->ADGDR &= LPC_ADC->ADGDR;							//clear the ADGDR bits
	ADC_EdgeStartConfig(LPC_ADC,ADC_START_ON_RISING);			//choose rising edge
	ADC_IntConfig(LPC_ADC,ADC_ADINTEN0,SET);					//set up and enable interruptions for adc channel 0
	NVIC_SetPriority(ADC_IRQn, 2);								//set a high priority for adc interruptions
