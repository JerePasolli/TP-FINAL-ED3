...
void timerConfig(void){
	...
	TIM_Init(LPC_TIM0, TIM_TIMER_MODE, (void*) &timCfg);	//timer initialization: select timer0 peripheral with timer mode and timCfg settings
	TIM_ConfigMatch(LPC_TIM0, &matchCfg);					//match register configuration for timer0
	TIM_Cmd(LPC_TIM0, ENABLE);								//enable timer0
